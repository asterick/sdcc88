...
- [minx-format.md](minx-format.md) — the **MINX container** (`.minx`), `romgen`'s second output format: the
  ROM image + a binary symbol table + the verbatim `.map`/`.noi`/`.cdb` debug artifacts in one ELF-like
  sectioned binary.
