...
- **`romgen`** (`tools/romgen.c`) — packs the linked banks into a flat `.min` ROM, or (with a `.minx`
  output / `--minx`) into the **MINX debug container**: a chunk-tree binary holding the ROM, symbols, a
  sorted source-line table, function extents, and the embedded source files — everything a debugging
  emulator needs with no filesystem and no text parsing (`docs/s1c88/minx-format.md`; validated/dumped
  by **`minxdump`**, `tools/minxdump.c`).
