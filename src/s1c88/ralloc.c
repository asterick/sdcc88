...
/*-----------------------------------------------------------------*/
void
s1c88SpillThis (symbol *sym, bool force_spill)
{
  int i;
  ...
  /* if this is rematerializable or has a spillLocation
     ...
     for it */
  if (!(sym->remat || sym->usl.spillLoc) || (sym->usl.spillLoc && !sym->usl.spillLoc->onStack)) // s1c88 port currently only supports on-stack spill locations in code generation.
    createStackSpil (sym);
...
packRegsForAssign (iCode *ic, eBBlock *ebp)
{
  ...
  /* Otherwise, for now eliminate 8-bit temporary variables only.
     The S1C88 instructions operating directly on memory
...
serialRegMark (eBBlock ** ebbs, int count)
{
  ...
  s1c88_call_stack_size = 2; // Saving of register to stack temporarily.
  ...
  for (int i = 0; i < count; i++)
    {
      ...
      for (iCode *ic = ebbs[i]->sch; ic; ic = ic->next)
        {
          if ((ic->op == CALL || ic->op == PCALL) && ic->parmBytes + 5 > s1c88_call_stack_size)
            {
              sym_link *dtype = operandType (IC_LEFT (ic));
              sym_link *ftype = IS_FUNCPTR (dtype) ? dtype->next : dtype;

              /* 5 for saving all registers at call site + 2 for big return value */
              s1c88_call_stack_size = ic->parmBytes + 5 + 2 * (getSize (ftype->next) > 4);
            }
...
s1c88_assignRegisters (ebbIndex *ebbi)
{
  ...
  s1c88_extend_stack = s1c88_call_stack_size > 255;

  /* Invoke optimal register allocator */
  ic = s1c88_ralloc2_cc (ebbi);

  /* redo the offsets for stacked automatic variables */
  if (currFunc && !s1c88_extend_stack && currFunc->stack + s1c88_call_stack_size > 255)
    {
      _G.slocNum = 0;
