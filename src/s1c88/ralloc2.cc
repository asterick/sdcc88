...
extern "C"
{
  ...
  long int s1c88_call_stack_size;
  bool s1c88_extend_stack;
}
...
static bool Ainst_ok(const assignment &a, unsigned short int i, const G_t &G, const I_t &I)
{
  ...
  if(ic->op == IPUSH)
    {
      if (ia.registers[REG_XL][1] < 0 || ia.registers[REG_YL][1] < 0 && !s1c88_extend_stack)
  ...
  for(boost::tie(oi, oi_end) = G[i].operands.equal_range(OP_SYMBOL_CONST(o)->key); oi != oi_end; ++oi)
    {
      ...
      if(a.global[v] >= 0)
        { 
          sym->regs[I[v].byte] = s1c88_regs + a.global[v];   
          sym->nRegs = I[v].size;
        }
      else
        {
...
{
  // s1c88 code generation is very flexible, and can handle nearly anything (including variables where some bytes are spilt, while others are not).
  ...
  for(unsigned int v = 0; v < boost::num_vertices(I); v++)
    {
      ...
      if(winner.global[v] >= 0)
        sym->regs[I[v].byte] = s1c88_regs + winner.global[v];   
      else
        {
          sym->regs[I[v].byte] = 0;
          spilt = true;
