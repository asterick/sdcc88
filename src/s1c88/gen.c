/*-------------------------------------------------------------------------
  gen.c - code generator for S1C88.
...
bool s1c88_regs_used_as_parms_in_calls_from_current_function[YH_IDX + 1];
bool s1c88_regs_used_as_parms_in_pcalls_from_current_function[YH_IDX + 1];
...
s1c88_init_reg_asmop(asmop *aop, const signed char *regidx)
{
  ...
  for(int i = 0; regidx[i] >= 0; i++)
    {
      aop->aopu.bytes[i].byteu.reg = s1c88_regs + regidx[i];
      aop->regs[regidx[i]] = i;
...
s1c88_init_asmops (void)
{
  ...
  s1c88_init_reg_asmop(&asmop_x, (const signed char[]){XL_IDX, XH_IDX, -1});
  s1c88_init_reg_asmop(&asmop_y, (const signed char[]){YL_IDX, YH_IDX, -1});
  s1c88_init_reg_asmop(&asmop_xy, (const signed char[]){XL_IDX, XH_IDX, YL_IDX, YH_IDX, -1});
  s1c88_init_reg_asmop(&asmop_xyl, (const signed char[]){XL_IDX, XH_IDX, YL_IDX, -1});
  s1c88_init_reg_asmop(&asmop_yx, (const signed char[]){YL_IDX, YH_IDX, XL_IDX, XH_IDX, -1});
...
updateCFA (void)
{
  ...
  if (options.debug && !regalloc_dry_run)
    debugFile->writeFrameAddress (NULL, &s1c88_regs[SP_IDX], 1 + G.stack.param_offset + G.stack.pushed);
}
//
      ...
      if (soffset > 255)
        {
          ...
          wassertl_bt (regalloc_dry_run || s1c88_extend_stack, "Extended stack access, but y not prepared for extended stack access.");
...
regFree (int idx, const iCode *ic)
{
  ...
  if ((idx == YL_IDX || idx == YH_IDX) && s1c88_extend_stack)
    return FALSE;
...
genCall (const iCode *ic)
{
  ...
  if (bigreturn)
    {
      ...
      if (s1c88_extend_stack && !s1c88IsParmInCall(ftype, "y") && ic->op != PCALL && optimize.codeSpeed) // 6 bytes, 4 cycles.
        {
          emit2 ("addw", "y, #%d", stk + G.stack.size - 256 + G.stack.pushed);
          cost (4, 2);
          push (ASMOP_Y, 0, 2);
        }
      else if (!s1c88IsParmInCall(ftype, "x")) // 5 bytes, 5 cycles.
        {
      ...
      else if (!s1c88IsParmInCall(ftype, "y")) // 8 bytes, 6 cycles.
        {
      ...
      if ((options.model == MODEL_LARGE || IFFUNC_ISCOSMIC (ftype)) && left->aop->type == AOP_DIR)
        {
          ...
          adjustStack (prestackadjust, !s1c88IsParmInCall(ftype, "a"), !s1c88IsParmInCall(ftype, "x"), !s1c88IsParmInCall(ftype, "y"));
      ...
      else if (options.model == MODEL_LARGE || IFFUNC_ISCOSMIC (ftype))
        {
          ...
          if (aopInReg (left->aop, 0, X_IDX) || aopInReg (left->aop, 0, Y_IDX))
            push (left->aop, 0, 2);
          else if ((aopOnStackNotExt (left->aop, 0, 2) || !s1c88IsParmInCall(ftype, "y")) &&
            !aopInReg (left->aop, 2, XL_IDX) && !aopInReg (left->aop, 2, XH_IDX) &&
            !s1c88IsParmInCall(ftype, "x"))
            {
              genMove (ASMOP_X, left->aop, !aopInReg (left->aop, 2, A_IDX), true, !s1c88IsParmInCall(ftype, "y"));
              push (ASMOP_X, 0, 2);
            }
          else if (!s1c88IsParmInCall(ftype, "a") && left->aop->regs[A_IDX] < 1)
            {
              cheapMove (ASMOP_A, 0, left->aop, 0, false);
              push (ASMOP_A, 0, 1);
          ...
          else if (aopOnStackNotExt (left->aop, 1, 1) && !s1c88IsParmInCall(ftype, "y"))
            {
          ...
          if (!s1c88IsParmInCall(ftype, "a"))
            {
              cheapMove (ASMOP_A, 0, left->aop, 2, false);
              push (ASMOP_A, 0, 1);
            }
          else if (!s1c88IsParmInCall(ftype, "xl"))
            {
              cheapMove (ASMOP_X, 0, left->aop, 2, true);
              push (ASMOP_X, 0, 2);
              adjustStack (1, false, false, false);
            }
          else if (!s1c88IsParmInCall(ftype, "yl"))
            {
      ...
      else
        {
          ...
          else if ((aopInReg (left->aop, 0, Y_IDX) || s1c88IsParmInCall(ftype, "x")) && !s1c88IsParmInCall(ftype, "y")) // Faster than going through x.
            {
              genMove (ASMOP_Y, left->aop, !s1c88IsParmInCall(ftype, "a"), !s1c88IsParmInCall(ftype, "x"), true);

              adjustStack (prestackadjust, !s1c88IsParmInCall(ftype, "a"), !s1c88IsParmInCall(ftype, "x"), false);

              emit2 (jump ? "jp" : "call", "(y)");
              cost (2,jump ? 1 : 4);
            }
          else if (!s1c88IsParmInCall(ftype, "x"))
            {
              genMove (ASMOP_X, left->aop, !s1c88IsParmInCall(ftype, "a"), true, !s1c88IsParmInCall(ftype, "y"));
      ...
      if (currFunc && isFuncCalleeStackCleanup (currFunc->type) && prestackadjust && !IFFUNC_ISNORETURN (ftype)) // Copy return value into correct location on stack for tail call optimization.
        {
          ...
          bool use_y = s1c88IsParmInCall(ftype, "x");
  ...
  const bool result_in_frameptr = s1c88_extend_stack && SomethingReturned && !bigreturn && (aopRet (ftype)->regs[YL_IDX] >= 0 || aopRet (ftype)->regs[YH_IDX] >= 0);
  ...
  if (result_in_frameptr)
    {
      ...
      if (result->size == 1 && aopInReg (result, 0, XL_IDX) && aopInReg (aopRet (ftype), 0, YL_IDX) ||
        result->size == 2 && aopInReg (aopRet (ftype), 0, Y_IDX) && result_in_extstk)
        {
          ...
          emit2 ("exgw", "x, y");
          cost (1, 1);
          genMove (result, ASMOP_X, true, true, !s1c88_extend_stack);
        }
      ...
      else if (result->size == 1)
        {
          ...
          pop (ASMOP_Y, 0, 2);
          genMove (result, ASMOP_A, true, true, !s1c88_extend_stack);
        }
  ...
  if (SomethingReturned && !bigreturn)
    {
      ...
      genMove (result, aopRet (ftype), true, true, !s1c88_extend_stack);
    }
...
restore:
  ...
  // Restore regs.
  if (!regDead (Y_IDX, ic) && !s1c88_extend_stack)
...
genFunction (iCode *ic)
{
  ...
  D (emit2 (";", s1c88_assignment_optimal ? "Register assignment is optimal." : "Register assignment might be sub-optimal."));
  ...
  // For more information see sections titled "Unexpected DIV/DIVW instruction result in ISR" in various S1C88 errata notes (apparently all S1C88 are affected).
  ...
  if (IFFUNC_ISISR (sym->type) && !sym->funcDivFlagSafe)
    {
      ...
      cost (6, 5);
#else
      // The workaround obtained by further investigation of RFE #449. Experiments on S1C88S208MB and S1C88L152C6 show that div resets bit 6 of cc.
      if (!optimize.codeSize)
        emit3 (A_CLR, ASMOP_A, 0);	// Zero accumulator to reduce cycle cost in following division.
      emit2 ("div", "x, a");	// According to measurements on the S1C88S208MB and S1C88L152C6, div takes 2-3 cycles for divisions by zero and 2-17 cycles in general.
      cost (1, 3);
#endif
    }

  if (s1c88_extend_stack) // Setup for extended stack access.
    {
      G.stack.size = s1c88_call_stack_size + (sym->stack ? sym->stack : 0);
  ...
  if (options.debug && !regalloc_dry_run)
    debugFile->writeFrameAddress (NULL, &s1c88_regs[SP_IDX], 1);

  /* adjust the stack for the function */
  {
    int fadjust = -sym->stack;
    if (s1c88_extend_stack && G.stack.size - 256 >= 255 && // Using frame pointer speeds up stack adjustment (unless we add in x anyway).
    ...
    adjustStack (fadjust, true, true, !s1c88_extend_stack);
  }
}
...
genReturn (const iCode *ic)
{
  ...
  switch (size)
    ...
    case 3:
    case 4:
      wassertl (regalloc_dry_run || !s1c88_extend_stack, "Unimplemented 24-Bit/32-Bit return in function with extended stack access.");
    case 1:
    case 2:
      ...
      if (result->aop->type == AOP_REG) // Let genMove handle all the coalescing, swapw, exg, rlwa, etc).
        {
          ...
          idxarray[2] = -1;
          s1c88_init_reg_asmop (&swapped_aop, idxarray);
...
genSend (const iCode *ic)
{
  ...
  for (int i = 0; i < argreg->size; i++)
    if (!regalloc_dry_run)
       ((walk->op == PCALL) ? s1c88_regs_used_as_parms_in_pcalls_from_current_function : s1c88_regs_used_as_parms_in_calls_from_current_function)[argreg->aopu.bytes[i].byteu.reg->rIdx] = true;
...
/*---------------------------------------------------------------------*/
/* genS1C88Code - generate code for S1C88 for a single iCode instruction */
/*---------------------------------------------------------------------*/
static void
...
/*---------------------------------------------------------------------*/
/* genS1C88Code - generate code for S1C88 for a block of instructions    */
/*---------------------------------------------------------------------*/
void
genS1C88Code (iCode *lic)
{
  ...
  memset(s1c88_regs_used_as_parms_in_calls_from_current_function, 0, sizeof(bool) * (YH_IDX + 1));
  memset(s1c88_regs_used_as_parms_in_pcalls_from_current_function, 0, sizeof(bool) * (YH_IDX + 1));
...
{
  if (!strcmp(what, "x"))
    return (s1c88IsReturned ("xl") || s1c88IsReturned ("xh"));
  else if (!strcmp(what, "y"))
    return (s1c88IsReturned ("yl") || s1c88IsReturned ("yh"));
...
s1c88IsRegArg (struct sym_link *ftype, int i, const char *what)
{
  if (what && !strcmp(what, "x"))
    return (s1c88IsRegArg (ftype, i, "xl") || s1c88IsRegArg (ftype, i, "xh"));
  else if (what && !strcmp(what, "y"))
    return (s1c88IsRegArg (ftype, i, "yl") || s1c88IsRegArg (ftype, i, "yh"));
