...
isPairInUse (PAIR_ID id, const iCode * ic)
{
  if (id == PAIR_DE)
    ...
    {
      return bitVectBitValue (ic->rMask, B_IDX) || 0;
    }
  else if (id == PAIR_BA)
    {
...
isPairDead (PAIR_ID id, const iCode * ic)
{
  switch (id)
    ...
    case PAIR_DE:
      return true && true;
    case PAIR_BC:
      return isRegDead (B_IDX, ic) && true;
    case PAIR_HL:
  ...
  switch (op1type)
    ...
    case AOP_HL:
      ...
      switch (op2type)
        ...
        case AOP_LIT:
          if (count)
            cost2 (2, 10, 9, 7, 12, 8, 3, 3); // ld (hl), n
          return (5);
        case AOP_STK:
          if (count)
            {
  ...
  for (int i = 0; i < n && result->type == AOP_REG;)
    {
      ...
      else if (i + 1 < n && !assigned[i + 1] && (source->type == AOP_STK || source->type == AOP_EXSTK) && sp_offset == 2 && getPairId_o (result, roffset + i) != PAIR_INVALID && (getPairId_o (result, roffset + i) != PAIR_HL && hl_free || getPairId_o (result, roffset + i) != PAIR_DE && de_free) && (!regalloc_dry_run || source->aopu.aop_stk > 0) && !optimize.codeSpeed) // A bit slower (42 vs 38 cycles on Z80 and Z80N), so don't do it when optimizing for speed.
        {
          PAIR_ID pair = getPairId_o (result, roffset + i);
  ...
  for (int i = 0; i < size;)
    {
      ...
      else if (source->type == AOP_STL && !(soffset + i) && size == 2 && getPairId_o(result, roffset) == PAIR_DE) // For result in de, we don't need hl dead.
        {
          if (!hl_dead)
      ...
      if ((optimize.allow_unsafe_read || i + 1 == size && soffset + i + 1 <= source->size) && result->type == AOP_REG && !aopInReg (result, roffset + i, A_IDX) &&
        (i + 1 == size || soffset + i + 1 >= source->size) && (source->type == AOP_HL && fetchLitPair (PAIR_HL, source, soffset + i, f_dead, true) || source->type == AOP_IY))
        {
          ...
          if ((aopInReg (result, roffset + i, B_IDX)) && bc_dead && !a_dead)
            pair = PAIR_BC;
...
restoreRegs (bool iy, bool de, bool bc, bool hl, const operand *result, const iCode *const ic)
{
  ...
  wassert (!de); /* S1C88: no DE pair — nothing ever saves it */
    ...
    while (size)
      {
        ...
        bool c_free = true && (IC_LEFT (ic)->aop->regs[C_IDX] < 0 || IC_LEFT (ic)->aop->regs[C_IDX] >= size - 1);
        bool d_free = true && (IC_LEFT (ic)->aop->regs[D_IDX] < 0 || IC_LEFT (ic)->aop->regs[D_IDX] >= size - 1);
        bool e_free = true && (IC_LEFT (ic)->aop->regs[E_IDX] < 0 || IC_LEFT (ic)->aop->regs[E_IDX] >= size - 1);
        ...
        bool de_free = true && (d_free || IC_LEFT (ic)->aop->regs[D_IDX] >= size - 2) && (e_free || IC_LEFT (ic)->aop->regs[E_IDX] >= size - 2);
        bool bc_free = isRegDead (B_IDX, ic) && (b_free || IC_LEFT (ic)->aop->regs[B_IDX] >= size - 2) && (c_free || IC_LEFT (ic)->aop->regs[C_IDX] >= size - 2);
        ...
        // gbz80 flag handling differs from other z80 variants, allowing this hack to push a 16-bit zero.
          ...
          aopInReg (IC_LEFT (ic)->aop, size - 1, H_IDX) && l_free || h_free && aopInReg (IC_LEFT (ic)->aop, size - 2, L_IDX)))
          {
            ...
            genMove_o (pair, 0, IC_LEFT (ic)->aop, size - 2, 2, a_free, hl_free, true, true, true);
       ...
       else if (size >= 2)
         {
           ...
           cost2 (1, 11, 11, 10, 16, 8, 3, 4);
           _G.stack.pushed += 4;
           genMove_o (ASMOP_HL, 0, IC_LEFT (ic)->aop, size - 2, 2, a_free, true, true, true, true);
           emit2 ("ld %d (sp), hl", 2);
       ...
       else if (a_free || b_free || h_free || l_free)
         {
           ...
           genMove_o (tmp, 0, IC_LEFT (ic)->aop, size - 1, 1, a_free, h_free && l_free, true, iyh_free && iyl_free, true);
           emit2 ("push %s", tname);
           cost2 (2, 11, 11, 10, 16, 8, 3, 4);
           d = 1;
         }
           ...
           cost2 (1, 11, 11, 10, 16, 8, 3, 4);
           _G.stack.pushed += 4;
           genMove_o (ASMOP_H, 0, IC_LEFT (ic)->aop, size - 1, 1, false, false, true, iyh_free && iyl_free, true);
           emit2 ("ld %d (sp), hl", 2);
           cost2 (3, 0, 0, 0, 0, 0, 0, 0);
           emit2 ("pop hl");
           cost2 (1, 10, 9, 7, 12, 10, 3, 3);
...
genPointerPush (const iCode *ic)
{
  ...
  genMove (ASMOP_HL, IC_LEFT (ic)->aop, true, true, true, stash_hl ? false : isRegDead (IY_IDX, ic));
...
genCall (const iCode *ic)
{
  ...
  bool de_not_parm = !s1c88IsParmInCall(ftype, "e") && !s1c88IsParmInCall(ftype, "d");
  bool de_free = de_not_parm && ic->left->aop->regs[E_IDX] < 0 && ic->left->aop->regs[D_IDX] < 0;
  bool bc_not_parm = !s1c88IsParmInCall(ftype, "b") && !s1c88IsParmInCall(ftype, "c");
    ...
    {
      {
          ...
          adjustStack (-sym->stack, !s1c88IsParmInCall (sym->type, "a"), !s1c88IsParmInCall (sym->type, "c") && !s1c88IsParmInCall (sym->type, "v"), !s1c88IsParmInCall (sym->type, "e") && !s1c88IsParmInCall (sym->type, "d"), !s1c88IsParmInCall (sym->type, "l") && !s1c88IsParmInCall (sym->type, "h"), !IY_RESERVED);
        }
      _G.stack.pushed = 0;
    }
  else if (!_G.omitFramePtr)
...
genPlusIncr (const iCode *ic)
{
      ...
      if (!sameRegs (IC_LEFT (ic)->aop, IC_RESULT (ic)->aop))
        {
          if (icount > 3)
            return FALSE;
          if (!delayed_move)
            genMove (IC_RESULT (ic)->aop, IC_LEFT (ic)->aop, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true);
        }
      while (icount--)
        {
  ...
  if (size == 2 && icount <= 2 && isPairDead (PAIR_HL, ic) && (IC_LEFT (ic)->aop->type == AOP_HL || IC_LEFT (ic)->aop->type == AOP_IY))
    {
      genMove (ASMOP_HL, IC_LEFT (ic)->aop, isRegDead (A_IDX, ic), true, true, true);
      while (icount--)
        emit3w (A_INC, ASMOP_HL, 0);
      genMove (IC_RESULT (ic)->aop, ASMOP_HL, isRegDead (A_IDX, ic), true, true, true);
      return true;
...
setupToPreserveCarry (asmop *result, asmop *left, asmop *right, const iCode *ic)
{
  ...
  if (couldDestroyCarry (right) && couldDestroyCarry (result) && couldDestroyCarry (left) &&
    ...
    {
      genMove (result, left, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true);
...
genPlus (iCode * ic)
{
  ...
  if (!maskedtopbyte && (isPair (IC_RIGHT (ic)->aop) || isPair (IC_LEFT (ic)->aop)) && getPairId (IC_RESULT (ic)->aop) == PAIR_HL)
    {
      ...
      if (left == PAIR_HL && right != PAIR_INVALID && (right != PAIR_IY))
        ...
        {
          genMove_o (ASMOP_HL, 0, ic->left->aop, 0, 2, isRegDead (A_IDX, ic), true, true && right != PAIR_DE, isRegDead (IY_IDX, ic) && right != PAIR_IY, true);
          emit3w (A_ADD, ASMOP_HL, ic->right->aop);
          goto release;
        }
      else if (left != PAIR_INVALID && left != PAIR_HL && (left != PAIR_IY))
        {
          genMove_o (ASMOP_HL, 0, ic->right->aop, 0, 2, isRegDead (A_IDX, ic), true, true && left != PAIR_DE, isRegDead (IY_IDX, ic) && left != PAIR_IY, true);
          emit3w (A_ADD, ASMOP_HL, ic->left->aop);
          goto release;
        }
      else if (left == PAIR_HL && isPairDead (PAIR_BA, ic))
        {
          ...
          genMove (ASMOP_BA, ic->right->aop, false, false, false, false);
          emit2 ("add hl, ba");
          cost2 (1, 11, 7, 2, 8, 8, 1, 1);
          goto release;
        }
        ...
        {
          genMove (ASMOP_BA, leftop, false, false, false, false);
          emit2 ("add hl, ba");
          cost2 (1, 11, 7, 2, 8, 8, 1, 1);
          goto release;
        }
    ...
    (ic->right->aop->type == AOP_LIT || ic->right->aop->type == AOP_IMMD || ic->left->aop->type == AOP_IMMD && (ic->right->aop->type == AOP_HL || ic->right->aop->type == AOP_IY)))
    {
      ...
      genMove (ASMOP_HL, ic->left->aop, isRegDead (A_IDX, ic), true, true, isRegDead (IY_IDX, ic));
      genMove (ASMOP_BA, ic->right->aop, isRegDead (A_IDX, ic), false, true, isRegDead (IY_IDX, ic));
      emit2 ("add hl, ba");
      cost2 (1, 11, 7, 2, 8, 8, 1, 1);
      goto release;
    }
  ...
  if (!maskedtopbyte && IC_RESULT (ic)->aop->type == AOP_EXSTK && size <= 2 && (getPairId (IC_LEFT (ic)->aop) == PAIR_HL || getPairId (IC_RIGHT (ic)->aop) == PAIR_HL) &&
    ...
    ic->left->aop->type != AOP_IY && ic->right->aop->type != AOP_IY)
    {
      ...
      genMove (ASMOP_IY, IC_LEFT (ic)->aop, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true);
      if (save_pair)
        _push (pair);
      asmop *raop = (pair == PAIR_HL) ? ASMOP_HL : ASMOP_BA;
      genMove (raop, ic->right->aop, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true);
  ...
  for (i = 0, started = false; i < size;)
    {
      ...
      if (!maskedword && leftop->type == AOP_STL && !i && i + 1 < size && rightop->type == AOP_LIT && hl_dead)
      ...
      else if (!maskedword && leftop->type == AOP_STL && !i && i + 1 < size && hl_dead && (size <= 2 || leftop->type != AOP_EXSTK /* (hl) would be pointed to result, overwritten by addition here */))
        {
          ...
          if (save_ba)
            _push (PAIR_BA);
          genMove (ASMOP_BA, rightop, true, true, true, false);
          genMove (ASMOP_HL, leftop, true, true, true, false);
          emit2 ("add hl, ba");
          spillPair (PAIR_BA);
          cost2 (1, 11, 7, 2, 8, 8, 1, 1);
          started = true;
      ...
      else if (!maskedbyte && (!premoved || i) && optimize.codeSize && !started && i == size - 1 && isPairDead (PAIR_HL, ic) && rightop->type == AOP_LIT && aopInReg (IC_RESULT (ic)->aop, i, L_IDX) && aopInReg (leftop, i, L_IDX))
        {
          PAIR_ID pair = !true ? PAIR_DE : PAIR_BC;
      ...
      // Conditional 16-bit inc.
      else if (!maskedword && i == size - 2 && started && aopIsLitVal (rightop, i, 2, 0) && (aopInReg (IC_RESULT (ic)->aop, i, HL_IDX) && aopInReg (leftop, i, HL_IDX) || aopInReg (IC_RESULT (ic)->aop, i, IY_IDX) && aopInReg (leftop, i, IY_IDX)))
        {
          ...
          cost2 (2, 12, 8, 5, 12, 12, 3, 3); // Assume branch is taken. Use cost of jr as the peephole optimizer can typically optimize this jp into jr. Do not emit jr directly to still allow jump-to-jump optimization.
  ...
  while (size)
    {
      ...
      if (!maskedword && size >= 2 && isPairDead (PAIR_HL, ic) && (aopInReg (result, offset, HL_IDX)) && (result->regs[L_IDX] < 0 || result->regs[L_IDX] >= offset) && (result->regs[H_IDX] < 0 || result->regs[H_IDX] >= offset) && (aopInReg (left, offset, HL_IDX) || (left->type == AOP_LIT || left->type == AOP_IY) && right->regs[L_IDX] < offset && right->regs[H_IDX] < offset) && (((right->type == AOP_IY || right->type == AOP_HL) && (getFreePairId (ic) == PAIR_DE || getFreePairId (ic) == PAIR_BC))))
        {
          PAIR_ID rightpair;
...
genMultOneChar (const iCode * ic)
{
  ...
  genMove (result, ASMOP_HL, true, true, true, true);
...
no_mlt:
  ...
  genMove (IC_RESULT (ic)->aop, add_in_hl ? ASMOP_HL : ASMOP_A, true, add_in_hl || isPairDead (PAIR_HL, ic), true, true);
          ...
          if (sign)             /* Map signed operands to unsigned ones. This pre-subtraction workaround to lack of signed comparison is cheaper than the post-subtraction one at fix. */
            {
              ...
              if (size == 2 && !(!ifx && requiresHL(result->aop) && result->aop->type != AOP_REG) && isPairDead (PAIR_HL, ic) && (getPairId (left->aop) == PAIR_HL))
                {
                  PAIR_ID litpair = (true ? PAIR_DE : PAIR_BC);
                  fetchPair (PAIR_HL, left->aop);
      ...
      while (size)
        {
          ...
          else if (size >= 2 && (!sign || size > 2) && !left_already_in_a && isPairDead (PAIR_HL, ic) && (getPartPairId (left->aop, offset) == PAIR_HL || (left->aop->type == AOP_LIT || left->aop->type == AOP_IMMD || left->aop->type == AOP_HL || left->aop->type == AOP_IY) && right->aop->regs[L_IDX] < offset && right->aop->regs[H_IDX] < offset) && (getPartPairId (right->aop, offset) == PAIR_DE || getPartPairId (right->aop, offset) == PAIR_BC))
            {
          ...
          else if (size >= 2 && (!sign || size > 2) && !left_already_in_a && isPairDead (PAIR_HL, ic) && true && left->aop->regs[E_IDX] < offset + 1 && left->aop->regs[D_IDX] < offset + 1 && (getPartPairId (left->aop, offset) == PAIR_HL || left->aop->type == AOP_LIT || left->aop->type == AOP_IMMD || left->aop->type == AOP_HL || left->aop->type == AOP_IY) && (right->aop->type == AOP_LIT || right->aop->type == AOP_IMMD || right->aop->type == AOP_HL || right->aop->type == AOP_IY))
            {
...
genCmpEq (iCode * ic, iCode * ifx)
{
  ...
  if (left->aop->type == AOP_CRY && ((right->aop->type == AOP_CRY) || (right->aop->type == AOP_LIT)))
  ...
  else
    {
      ...
      if (result->aop->type != AOP_CRY)
        genMove (result->aop, ASMOP_A, true, isPairDead (PAIR_HL, ic), true, true);
    }
  ...
  for (int i = 0; i < size;)
    {
      ...
      if (aopIsLitVal (result->aop, i, 1, 0x00) || aopIsLitVal (right->aop, i, 1, 0x00) || aopIsLitVal (right->aop, i, 1, 0xff))
        {
          ...
          for(end = i; end < size && (!bytelit && aopIsLitVal (result->aop, end, 1, 0x00) || aopIsLitVal (right->aop, end, 1, bytelit)); end++);
            genMove_o (result->aop, i, bytelit == 0x00 ? ASMOP_ZERO : left->aop, i, end - i, a_free, hl_free, !false, true, true);
      ...
      genMove_o (result->aop, i, ASMOP_A, 0, 1, true, hl_free, !false, true, true);
      ...
      if (!requiresHL (right->aop) || (result->aop->regs[L_IDX] < 0 && result->aop->regs[H_IDX] < 0))
        { /* only if not (right requires HL and result use HL) */
          genMove (result->aop, left->aop,
                   isRegDead (A_IDX, ic),
                   isPairDead (PAIR_HL, ic),
                   true && right->aop->regs[D_IDX] < 0 && right->aop->regs[E_IDX] < 0,
                   true);
          left = result;
        }
    }
      ...
      if (left->aop->type == AOP_REG && right->aop->type == AOP_REG && aopIsLitVal (left->aop, i, 1, 0x00) && aopIsLitVal (right->aop, i + 1, 1, 0x00) && (aopInReg (right->aop, i, L_IDX) && aopInReg (left->aop, i + 1, H_IDX) || aopInReg (right->aop, i, IYL_IDX) && aopInReg (left->aop, i + 1, IYH_IDX)))
        {
          ...
          genMove_o (result->aop, i, source, 0, 2, isRegDead (A_IDX, ic), isRegDead (HL_IDX, ic), true, isRegDead (IY_IDX, ic), true);
          i += 2;
          continue;
        }
      ...
      else if (aopIsLitVal (left->aop, i, 1, 0x00) && !pushed_a)
        {
          ...
          for(end = i; end < size && aopIsLitVal (left->aop, end, 1, 0x00); end++);
            genMove_o (result->aop, i, right->aop, i, end - i, a_free, hl_free, !false, true, true);
          if (result->aop->regs[A_IDX] >= i && result->aop->regs[A_IDX] < end)
            a_free = false;
          i = end;
          continue;
      ...
      else if (aopIsLitVal (right->aop, i, 1, 0x00) && !pushed_a)
        {
          ...
          for(end = i; end < size && aopIsLitVal (right->aop, end, 1, 0x00); end++);
            genMove_o (result->aop, i, left->aop, i, end - i, a_free, hl_free, !false, true, true);
          if (result->aop->regs[A_IDX] >= i && result->aop->regs[A_IDX] < end)
            a_free = false;
          i = end;
          continue;
      ...
      else if (aopIsLitVal (left->aop, i, 1, 0xff) || aopIsLitVal (right->aop, i, 1, 0xff))
        {
          ...
          for(end = i; end < size && (aopIsLitVal (left->aop, end, 1, 0xff) || aopIsLitVal (right->aop, end, 1, 0xff)); end++);
            genMove_o (result->aop, i, ASMOP_MONE, i, end - i, a_free, hl_free, !false, true, true);
          if (result->aop->regs[A_IDX] >= i && result->aop->regs[A_IDX] < end)
            a_free = false;
          i = end;
          continue;
    ...
    for (int i = 0; i < size;)
      {
        ...
        if (aopIsLitVal (right_aop, i, 1, 0x00))
          {
            ...
            genMove_o (result_aop, i, left_aop, i, end - i, a_free, hl_free, !false, true, true);
        ...
        genMove_o (result_aop, i, ASMOP_A, 0, 1, true, hl_free, !false, true, true);
      ...
      if (left->aop->type != AOP_REG && !operandsEqu (result, left))
        {
          ...
          genMove_o (result->aop, 0, left->aop, 0, size, true, isPairDead (PAIR_HL, ic), true, true, true);
          left = result;
        }
      cheapMove (ASMOP_A, 0, left->aop, offset, true);
  ...
  if (left->aop->type == AOP_REG || result->aop->type == AOP_STK ||
           ...
           result->aop->type == AOP_EXSTK || result->aop->type == AOP_REG)
    {
      ...
      if (size == 2 && (aopInReg (left->aop, 0, HL_IDX) && isRegDead (HL_IDX, ic)))
        rotaop = left->aop;
      genMove (rotaop, left->aop, true, isRegDead (HL_IDX, ic), true, isRegDead (IY_IDX, ic));
     ...
     genMove (result->aop, rotaop, true, isRegDead (HL_IDX, ic), true, isRegDead (IY_IDX, ic));
    }
  else
    {
      if (!isRegDead (A_IDX, ic))
...
genGetByte (const iCode *ic)
{
  ...
  genMove_o (result->aop, 0, left->aop, offset, 1, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true, true);
...
genGetWord (const iCode *ic)
{
  ...
  offset = (int) ulFromVal (right->aop->aopu.aop_lit) / 8;
  genMove_o (result->aop, 0, left->aop, offset, 2, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true, true);
...
shiftR2Left2Result (const iCode *ic, operand *left, int offl, operand *result, int offr, int shCount, int is_signed)
{
  ...
  if (!is_signed && aopSame (result->aop, offr, left->aop, offl, 2) && isPairDead (PAIR_HL, ic) && isRegDead (A_IDX, ic) &&
    ...
    {
      tlbl = regalloc_dry_run ? 0 : newiTempLabel (NULL);
      genMove (ASMOP_HL, left->aop, isRegDead (A_IDX, ic), true, true, isRegDead (IY_IDX, ic));
      emit3w (A_ADD, ASMOP_HL, ASMOP_HL);
  ...
  // If the leading bits are all the same, we can shift the other way, and use efficient 16-bit addition for shifts.
    ...
    shCount >= 5 - !optimize.codeSpeed) // Smaller code size for 4 and above, but at least for Z80(N), only faster from 5.
    {
      ...
      genMove_o (result->aop, 1, ASMOP_A, 0, 1, true, false, true, isPairDead (PAIR_IY, ic), true);
      return;
    }
  ...
  if (isPair (result->aop) && !offr)
    ...
    genMove_o (result->aop, offr, left->aop, offl, 2, true, isPairDead (PAIR_HL, ic), true, true, true);
...
shiftL2Left2Result (operand *left, operand *result, int shCount, const iCode *ic)
{
  ...
  if ((result->aop->type == AOP_HL || result->aop->type == AOP_IY) && (left->aop->type == AOP_HL || left->aop->type == AOP_IY) && isPairDead (PAIR_HL, ic) && (shCount > 1 || !sameRegs (result->aop, left->aop)) || isPairDead (PAIR_HL, ic) && getPairId (result->aop) == PAIR_DE && getPairId (left->aop) != PAIR_DE) // Shift in hl if we can cheaply move to de via ex later.
    {
      shiftaop = ASMOP_HL;
      genMove (ASMOP_HL, left->aop, isRegDead (A_IDX, ic), true, true, true);
    }
    ...
    genMove_o (result->aop, 0, left->aop, 0, 2, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true, true);
  ...
  if (shiftaop != result->aop)
    {
      if (isPair (result->aop))
        ...
        genMove_o (result->aop, 0, shiftaop, 0, 2, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true, true);
    }
}
...
genLeftShiftLiteral (operand *left, operand *right, operand *result, const iCode *ic)
{
  ...
  if (shCount >= (size * 8))
    genMove (result->aop, ASMOP_ZERO, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic) && left->aop->regs[H_IDX] < 0 && left->aop->regs[L_IDX] < 0, true && left->aop->regs[D_IDX] < 0 && left->aop->regs[E_IDX] < 0, true);
  else
    {
      switch (size)
        {
...
genLeftShift (const iCode *ic)
{
  ...
  else if (true && result->aop->regs[C_IDX] < 0 && left->aop->regs[C_IDX] < 0)
    countreg = C_IDX;
  else {
      UNIMPLEMENTED;
      countreg = A_IDX;
  ...
  if (!shift_by_lit && count_iy)
    genMove (ASMOP_IY, right->aop, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true);
  ...
  if (!sameRegs (shiftop, left->aop) || shiftop->type == AOP_REG)
    {
      ...
      bool de_dead = true && (countreg != E_IDX && countreg != D_IDX || shift_by_lit);
  ...
  while (size)
    {
      if (size >= 2 && offset + 1 >= byteshift && shiftop->type == AOP_REG && (getPartPairId (shiftop, offset) == PAIR_HL || !started && getPartPairId (shiftop, offset) == PAIR_IY || (optimize.codeSize && !started) && getPartPairId (shiftop, offset) == PAIR_DE))
        {
  ...
  genMove_o (result->aop, 0, shiftop, 0, result->aop->size, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true, true);
...
genPointerGet (const iCode *ic)
{
  ...
  if (!bit_field && (result->aop->type == AOP_STK || result->aop->type == AOP_EXSTK) && size > 2 && size <= 255)
    {
      ...
      if (left->aop->type == AOP_IMMD)
        ...
        {
          genMove (ASMOP_HL, left->aop, true, true, true, true);
          if (rightval)
            offsetPair (PAIR_HL, PAIR_BC, false, rightval);
        }
  ...
  extrapair = true ? PAIR_DE : PAIR_BC;
  ...
  if (!bit_field && isLitWord (result->aop) && size == 2 && offset == 0 && !sameRegs (result->aop, right->aop) && (right->aop->type == AOP_REG && getPairId (right->aop) != PAIR_INVALID || isLitWord (right->aop)))
    {
      if (isLitWord (right->aop))
        {
          pairId = isPairDead (PAIR_HL, ic) ? PAIR_HL :  true ? PAIR_DE : PAIR_BC;
  ...
  if (bit_field && getPairId (result->aop) != PAIR_INVALID && (getPairId (result->aop) != PAIR_IY || SPEC_BLEN (getSpec (operandType (result)->next)) < 8 || isPairDead (getPairId (result->aop), ic)))   /* Avoid destroying result by increments */
  ...
  else
    {
      ...
      genMove (pairId == PAIR_HL ? ASMOP_HL : pairId == PAIR_DE ? ASMOP_DE : ASMOP_BC, result->aop,
        ...
        true && right->aop->regs[E_IDX] < 0 && right->aop->regs[D_IDX] < 0,
...
genAssign (const iCode *ic)
{
  ...
  if (isPair (result->aop) && getPairId (result->aop) != PAIR_IY ||
    ...
    genMove (result->aop, right->aop, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), true, true);
  else if (size == 2 && isPairDead (PAIR_HL, ic) && ((right->aop->type == AOP_STK && !_G.omitFramePtr || right->aop->type == AOP_IY || right->aop->type == AOP_LIT) && result->aop->type == AOP_IY || right->aop->type == AOP_IY && (result->aop->type == AOP_STK && !_G.omitFramePtr || result->aop->type == AOP_IY) || right->aop->type == AOP_LIT && (result->aop->type == AOP_STK || result->aop->type == AOP_EXSTK) && (result->aop->aopu.aop_stk + offset + _G.stack.offset + (result->aop->aopu.aop_stk > 0 ? _G.stack.param_offset : 0) + _G.stack.pushed) == 0)) // Use ld d(sp), hl
    {
      ...
      genMove (result->aop, ASMOP_HL, isRegDead (A_IDX, ic), true, true, isPairDead (PAIR_IY, ic));
    }
...
genJumpTab (const iCode *ic)
{
  ...
  if ((pair = getDeadPairId (ic)) == PAIR_INVALID)
    pair = PAIR_DE;
  ...
  genMove (pair == PAIR_DE ? ASMOP_DE : ASMOP_BC, jtcond->aop, isRegDead (A_IDX, ic), true, true, isPairDead (PAIR_IY, ic));
...
genCast (const iCode *ic)
{
  ...
  if (IS_BITINT (resulttype) && (SPEC_BITINTWIDTH (resulttype) % 8) && bitsForType (resulttype) < bitsForType (righttype))
    {
      if (!isRegDead (A_IDX, ic) || result->aop->regs[A_IDX] >= 0 && result->aop->regs[A_IDX] != result->aop->size - 1)
        _push (PAIR_AF), pushed_a = true;
      genMove (result->aop, right->aop, true, isPairDead (PAIR_HL, ic), true, isPairDead (PAIR_IY, ic));
      cheapMove (ASMOP_A, 0, result->aop, result->aop->size - 1, true);
      emit2 ("and a, #0x%02x", topbytemask);
      cost2 (2, 7, 6, 4, 8, 4, 2, 2);
  ...
  if (IS_BOOL (righttype) || !IS_SPEC (righttype) || SPEC_USIGN (righttype) || right->aop->type == AOP_CRY)
    {
      genMove_o (result->aop, 0, right->aop, 0, right->aop->size, !surviving_a, isPairDead (PAIR_HL, ic), true, isPairDead (PAIR_IY, ic), true);
      ...
      bool de_dead = true && (result->aop->regs[E_IDX] < 0 || result->aop->regs[E_IDX] >= right->aop->size) && (result->aop->regs[D_IDX] < 0 || result->aop->regs[D_IDX] >= right->aop->size);
  ...
  else
    {
      ...
      genMove_o (result->aop, 0, right->aop, 0, right->aop->size - 1, !surviving_a, isPairDead (PAIR_HL, ic), true, isPairDead (PAIR_IY, ic), true);
      bool de_dead = true && (result->aop->regs[E_IDX] < 0 || result->aop->regs[E_IDX] >= right->aop->size) && (result->aop->regs[D_IDX] < 0 || result->aop->regs[D_IDX] >= right->aop->size);
      bool iy_dead = true && (result->aop->regs[IYL_IDX] < 0 || result->aop->regs[IYL_IDX] >= right->aop->size) && (result->aop->regs[IYH_IDX] < 0 || result->aop->regs[IYH_IDX] >= right->aop->size);
      bool hl_dead = isPairDead (PAIR_HL, ic) && (result->aop->regs[L_IDX] < 0 || result->aop->regs[L_IDX] >= right->aop->size) && (result->aop->regs[H_IDX] < 0 || result->aop->regs[H_IDX] >= right->aop->size);
