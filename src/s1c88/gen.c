...
skip_byte_push_iy:
...

  {
      ...
      for (int i = 0; i < n; i++)
        {
          if (assigned[i] && (aopInReg (result, roffset + i, L_IDX) || aopInReg (result, roffset + i, H_IDX)))
            no = true;
...
genPointerPush (const iCode *ic)
{
  ...
  genMove (ASMOP_HL, IC_LEFT (ic)->aop, true, true, swap_de ? false : true, isRegDead (IY_IDX, ic));
