/*-------------------------------------------------------------------------
  main.c - S1C88 specific definitions.
...
static int
s1c88_do_pragma (int id, const char *name, const char *cp)
{
...
static struct pragma_s s1c88_pragma_tbl[] = {
  {"codeseg", P_CODESEG, 0, s1c88_do_pragma},
  {"constseg", P_CONSTSEG, 0, s1c88_do_pragma},
  {NULL, 0, 0, NULL},
};

static int
s1c88_process_pragma (const char *s)
{
  return process_pragma_tbl (s1c88_pragma_tbl, s);
}

static char s1c88_defaultRules[] = {
#include "peeph.rul"
  ""
};


static char *s1c88_keywords[] = {
  "at",
  "critical",
  "interrupt",
  "trap",
...
static void
s1c88_reset_regparm (struct sym_link *ftype)
{
  _G.regparam.n = 0;
...
static int
s1c88_reg_parm (sym_link *l, bool reentrant)
{
...
static bool
s1c88_parseOptions (int *pargc, char **argv, int *i)
{
...
static const char *
s1c88_getRegName (const struct reg_info *reg)
{
  if (reg)
    return reg->name;
  return "err";
...
static void
s1c88_genExtraArea (FILE *of, bool hasMain)
{
...
s1c88_genInitStartup (FILE *of)
{
  ...
  fprintf (of, "; s1c88_genXINIT() start\n");
  ...
  fprintf (of, "00004$:\n");
  fprintf (of, "; s1c88_genXINIT() end\n");
}

#define S1C88_INTERRUPTS_COUNT 30

int
s1c88_genIVT(struct dbuf_s * oBuf, symbol ** intTable, int intCount)
{
  int i;
  dbuf_tprintf (oBuf, "\tint s_GSINIT ; reset\n");

  if(intCount > S1C88_INTERRUPTS_COUNT)
    {
      werror(E_INT_BAD_INTNO, intCount - 1);
      intCount = S1C88_INTERRUPTS_COUNT;
    }
...
/*----------------------------------------------------------------------*/
/* s1c88_dwarfRegNum - return the DWARF register number for a register.  */
/*----------------------------------------------------------------------*/
static int
s1c88_dwarfRegNum (const struct reg_info *reg)
{
  return reg->rIdx;
}
...
{
  "sdlds1c88", "-nf", "\"$1\"", "$L", NULL
};

/* $3 is replaced by assembler.debug_opts resp. port->assembler.plain_opts */
static const char *s1c88AsmCmd[] =
{
  "sdass1c88", "$l", "$3", "\"$1.asm\"", NULL
};
...
PORT s1c88_port =
{
  ...
  {
    glue,
    TRUE,                       /* We want s1c88_genIVT to be triggered */
  ...
  s1c88_options,
  NULL,
  s1c88_finaliseOptions,
  s1c88_setDefaultOptions,
  ...
  s1c88_keywords,
  NULL,
  s1c88_genAssemblerEnd,
  s1c88_genIVT,
  0,                            /* no genXINIT code */
  s1c88_genInitStartup,          /* genInitStartup */
  s1c88_reset_regparm,
  s1c88_reg_parm,
  s1c88_process_pragma,          /* process_pragma */
